...
**연관 시스템**:
- `products` 테이블 ← 상품 데이터 (is_live_active=true)
- `product_variants` 테이블 ← Variant 정보
- `useAuth` 훅 ← 인증 상태
- `useRealtimeProducts` 훅 ← 실시간 상품 구독
...
useRealtimeProducts hook
  ↓
products 테이블 (is_live_active=true)
  ↓
getProductVariants (병렬 로드)
  ↓
