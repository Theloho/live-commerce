...
**배포 상태**: ✅ **로컬 빌드 완료** (2.9초, 0 에러) + DB 마이그레이션 완료
**테스트 필요**: Vercel 배포 후 https://allok.shop/orders 검증 필요

**버그 타입**: UI 버그 (Rule #0-A 1번 타입)

**문제 상황**:
- 주문 내역 페이지에서 **UUID 표시** (`327bb989-...`) → product_number가 나와야 함
- **이미지 미표시** → thumbnail_url이 나와야 함
- 근본 원인: 성능 최적화 시 products JOIN 제거 → order_items에 데이터 없음

**근본 원인 분석** (Rule #0-A Stage 3):
1. ❌ **GetOrdersUseCase products JOIN 제거** (성능 최적화 시)
   - `order_items (*, products!...)` → `order_items (*)`
2. ❌ **order_items 테이블에 thumbnail_url, product_number 없음**
   - DB 스키마에 해당 컬럼 없음
3. ❌ **_normalizeOrders() fallback 작동**
   - `product_number: i.product_number || i.product_id` → UUID 표시

**해결 방법** (옵션 2: DB 마이그레이션 ⭐ 장기 해결):
1. ✅ **order_items 테이블 스키마 변경**
   ```sql
   ALTER TABLE order_items
     ADD COLUMN thumbnail_url TEXT,
     ADD COLUMN product_number VARCHAR(20);
   ```
2. ✅ **기존 데이터 마이그레이션**
   - products 테이블에서 thumbnail_url, product_number 복사
   - 인덱스 추가 (idx_order_items_product_number)
3. ✅ **CreateOrderUseCase _createItem() 수정**
   - 주문 생성 시 thumbnail_url, product_number 자동 저장
   - orderData에 없으면 products 테이블에서 가져오기
4. ✅ **DB_REFERENCE_GUIDE.md 업데이트**
   - order_items 스키마 반영

**성능 영향**:
- ✅ products JOIN 제거 유지 (성능 20배 향상 유지)
- ✅ 주문 조회 성능: 0.5-1초 유지
- ✅ 새 주문 생성: +1 SELECT 쿼리 (orderData에 없을 때만, 대부분 0회)
...
**다음 단계**: Git 커밋 → Vercel 배포 → 주문 내역 검증
...
**작업 시간**: 2025-10-22 오후 (Rule #0-A 적용)
**배포 상태**: ✅ **Git 커밋 완료** (8762b88)
**테스트 상태**: ⚠️ **버그 발견** → 즉시 수정 완료 (위 섹션 참조)
