...
- 🔧 **관리자 주문 관리 페이지 버그 수정** (2025-10-15 ⭐⭐)
  ...
  - **조사 중인 문제**:
    - ⚠️ G251015-8418 주문 고객에게는 보이지만 관리자에게 안 보임
    - 원인: S/G 주문번호 시스템 복잡도 (G = 그룹 결제, DB에 없음)
    - 디버깅 로그 추가: fb64740, 8a2787a
  - **내일 작업 계획** ⭐ 중요:
    - 🎯 주문 시스템 근본적 단순화 (1주문 = 1주문번호)
    - S/G 구분 제거, payment_group_id 단순화
    - 장바구니 결제 → 1개 주문 + order_items로 통합
    - 예상 시간: 2-3시간 (영구적 해결)
