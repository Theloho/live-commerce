...
/**
 * 새 쿠폰 생성 (관리자 전용 - Service Role API 사용)
...
export async function createCoupon(couponData) {
  ...
  // 관리자 전용 Service Role API 호출 (RLS 우회)
  ...
  if (!response.ok) {
    if (isDevelopment) console.error('❌ 쿠폰 생성 실패:', result)
    throw new Error(result.error || '쿠폰 생성에 실패했습니다')
  }

  if (isDevelopment) console.log('✅ 쿠폰 생성 완료:', result.coupon.code)
...
/**
 * 쿠폰 수정 (관리자 전용 - Service Role API 사용)
 * @param {string} couponId - 쿠폰 ID
...
export async function updateCoupon(couponId, updates) {
  ...
  // 관리자 전용 Service Role API 호출 (RLS 우회)
  const response = await fetch('/api/admin/coupons/update', {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      couponId,
  ...
  if (!response.ok) {
    if (isDevelopment) console.error('❌ 쿠폰 수정 실패:', result)
    throw new Error(result.error || '쿠폰 수정에 실패했습니다')
  }

  if (isDevelopment) console.log('✅ 쿠폰 수정 완료:', result.coupon.code)
  return result.coupon
}
...
/**
 * 쿠폰 완전 삭제 (관리자 전용 - Service Role API 사용)
 * ⚠️ CASCADE로 user_coupons도 삭제되지만, 주문 금액은 영향 없음
 * @param {string} couponId - 쿠폰 ID
 * @returns {Object} 삭제된 쿠폰 정보
 */
export async function deleteCoupon(couponId) {
  if (isDevelopment) console.log('🗑️ 쿠폰 완전 삭제:', couponId)
  ...
  if (!response.ok) {
    if (isDevelopment) console.error('❌ 쿠폰 삭제 실패:', result)
    throw new Error(result.error || '쿠폰 삭제에 실패했습니다')
  }

  if (isDevelopment) console.log('✅ 쿠폰 삭제 완료:', result.deletedCoupon.code)
  return result.deletedCoupon
}
...
/**
 * 특정 사용자에게 쿠폰 배포 (관리자 전용 - API Route 사용)
 * @param {string} couponId - 쿠폰 ID
 * @param {Array<string>} userIds - 사용자 ID 배열
 * @param {string} adminEmail - 관리자 이메일 (필수, useAdminAuth에서 전달)
...
export async function distributeCoupon(couponId, userIds, adminEmail) {
  ...
  // 관리자 전용 API Route 호출 (Service Role 사용)
  ...
  if (!response.ok) {
    if (isDevelopment) console.error('❌ 쿠폰 배포 실패:', result)
    throw new Error(result.error || '쿠폰 배포에 실패했습니다')
  }

  if (isDevelopment) console.log(`✅ 쿠폰 배포 완료: ${result.distributedCount}명에게 배포됨 (중복 ${result.duplicates}건 제외)`)
...
export async function getCouponHolders(couponId, filters = {}) {
  ...
  if (data && data.length > 0) {
    ...
    if (userIds.length > 0) {
      ...
      // user 정보 추가
      data.forEach(holder => {
        holder.user = profileMap[holder.user_id] || null
      })
    }
  }

  // ✅ order_id로 orders 별도 조회
  if (data && data.length > 0) {
    const orderIds = [...new Set(data.map(h => h.order_id).filter(Boolean))]
    ...
    if (orderIds.length > 0) {
      ...
      // order 정보 추가
      data.forEach(holder => {
        holder.order = orderMap[holder.order_id] || null
      })
    }
  }
...
export async function getUserCoupons(userId, filters = {}) {
  ...
  if (isDevelopment) console.log(`✅ 쿠폰 ${data?.length || 0}개 조회 완료`)
  return data
}
...
export async function validateCoupon(couponCode, userId, orderAmount) {
  ...
  const { data, error } = await supabase
    .rpc('validate_coupon', {
      ...
      p_user_id: userId,
      p_product_amount: orderAmount  // ✅ 수정: p_order_amount → p_product_amount (SQL 함수 시그니처와 일치)
    })
...
export async function getCouponStats(couponId) {
  ...
  try {
    ...
    // 쿠폰 조회 실패 시 기본 통계 반환
    if (!coupon) {
      if (isDevelopment) console.error('❌ 쿠폰 조회 실패: 쿠폰을 찾을 수 없음')
      return {
        total_issued: 0,
        total_used: 0,
        usage_rate: 0,
    ...
    if (isDevelopment) console.error('❌ 쿠폰 통계 조회 실패:', error)
    // 에러 발생 시 기본 통계 반환
    return {
      total_issued: 0,
      total_used: 0,
