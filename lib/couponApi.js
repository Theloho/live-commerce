...
export async function createCoupon(couponData) {
  console.log('🎫 쿠폰 생성 시작:', couponData)
  ...
  console.log('✅ 쿠폰 생성 완료:', data.code)
  return data
}
...
export async function getCoupons(filters = {}) {
  ...
  console.log(`✅ 쿠폰 ${data.length}개 조회 완료`)
  return data
}
...
export async function getCoupon(couponId) {
  ...
  console.log('✅ 쿠폰 조회 완료:', data.code)
  return data
}
...
export async function getCouponByCode(code) {
  ...
  if (error) {
    console.error('❌ 쿠폰 코드 조회 실패:', error)
    return null
  }

  console.log('✅ 쿠폰 코드 조회 완료:', data.code)
  return data
}
...
export async function updateCoupon(couponId, updates) {
  console.log('✏️ 쿠폰 수정:', couponId, updates)
  ...
  console.log('✅ 쿠폰 수정 완료:', data.code)
  return data
}
...
export async function toggleCouponStatus(couponId, isActive) {
  console.log(`${isActive ? '✅' : '❌'} 쿠폰 ${isActive ? '활성화' : '비활성화'}:`, couponId)
...
export async function deleteCoupon(couponId) {
  console.log('🗑️ 쿠폰 삭제 (비활성화):', couponId)
...
export async function distributeCoupon(couponId, userIds) {
  console.log(`📮 쿠폰 배포: ${userIds.length}명에게 배포 시작`)
  ...
  console.log(`✅ 쿠폰 배포 완료: ${data?.length || 0}명에게 배포됨 (중복 제외)`)
...
export async function distributeToAllCustomers(couponId) {
  ...
  if (customersError) {
    console.error('❌ 고객 목록 조회 실패:', customersError)
    throw customersError
  }

  const userIds = customers.map(c => c.id)
  console.log(`👥 전체 고객 수: ${userIds.length}명`)
...
export async function getCouponHolders(couponId, filters = {}) {
  console.log('👥 쿠폰 보유 고객 조회:', couponId, filters)
  ...
  if (error) {
    console.error('❌ 쿠폰 보유 고객 조회 실패:', error)
    throw error
  }

  console.log(`✅ ${data.length}명 조회 완료`)
  return data
}
...
export async function getUserCoupons(userId, filters = {}) {
  console.log('🎫 사용자 보유 쿠폰 조회:', userId)
...
export async function validateCoupon(couponCode, userId, orderAmount) {
  console.log('🔍 쿠폰 유효성 검증:', { couponCode, userId, orderAmount })
  ...
  if (!result.is_valid) {
    console.log('❌ 쿠폰 사용 불가:', result.error_message)
  } else {
    console.log(`✅ 쿠폰 사용 가능! 할인 금액: ₩${result.discount_amount.toLocaleString()}`)
  }
...
export async function applyCouponUsage(userId, couponId, orderId, discountAmount) {
  console.log('💳 쿠폰 사용 처리:', { userId, couponId, orderId, discountAmount })
  ...
  if (!data) {
    console.error('❌ 쿠폰 사용 실패: 이미 사용되었거나 보유하지 않은 쿠폰')
    return false
  }
...
export async function getCouponStats(couponId) {
  console.log('📊 쿠폰 통계 조회:', couponId)

  const { data: coupon } = await getCoupon(couponId)
