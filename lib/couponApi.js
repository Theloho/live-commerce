...
export async function getUserCoupons(userId, filters = {}) {
  console.log('🎫 사용자 보유 쿠폰 조회 시작:', { userId, filters })
  ...
  console.log(`✅ 쿠폰 ${data?.length || 0}개 조회 완료`)

  // 🔍 상세 디버깅
  if (data && data.length > 0) {
    console.log('🔍 각 쿠폰의 is_used 상태:')
    data.forEach((uc, idx) => {
      console.log(`  [${idx}] ${uc.coupon?.code || 'NO_CODE'}: is_used=${uc.is_used}, used_at=${uc.used_at}, order_id=${uc.order_id}`)
    })
  } else {
    console.warn('⚠️ user_coupons 테이블에서 데이터를 찾지 못했습니다')
    console.log('🔍 RLS 정책 확인 필요 - user_id:', userId)
  }
