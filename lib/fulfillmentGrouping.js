...
function createGroupData(orders, type) {
  ...
  // 배송지 정보
  const shippingInfo = {
    name: firstOrder.shipping_name || firstOrder.order_shipping?.[0]?.name || '이름없음',
    phone: firstOrder.shipping_phone || firstOrder.order_shipping?.[0]?.phone || '연락처없음',
    postalCode: firstOrder.shipping_postal_code || firstOrder.order_shipping?.[0]?.postal_code || '',
    address: firstOrder.shipping_address || firstOrder.order_shipping?.[0]?.address || '주소없음',
  ...
  orders.forEach(order => {
    ...
    const items = order.order_items?.map(item => {
      ...
      return {
        ...
        productName: item.products?.title || item.title || '제품명 없음',
        productImage: item.products?.image_url || item.product_variants?.image_url || null,
...
export function generateGroupCSV(groups, selectedOrderIds) {
  const headers = ['그룹ID', '주문번호들', '고객명', '연락처', '주소', '제품목록', '옵션', 'SKU', '총수량', '총금액', '배송타입', '송장번호']
  ...
  const rows = groups.map(group => {
    ...
    const productNames = selectedItems.map(item => `${item.productName}(${item.quantity})`).join('; ')
...
export function generateOrderCSV(groups, selectedOrderIds) {
  const headers = ['주문번호', '고객명', '연락처', '주소', '제품명', '옵션', 'SKU', '수량', '금액', '그룹ID', '배송타입', '송장번호']
