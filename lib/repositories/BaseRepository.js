...
export class BaseRepository {
  /**
   * Supabase 클라이언트 (Service Role 또는 일반)
  ...
  /**
   * BaseRepository 생성자
   * @param {import('@supabase/supabase-js').SupabaseClient} client - Supabase 클라이언트
  ...
  constructor(client, tableName) {
    ...
    if (!this.client) {
      throw new Error('client must be provided in constructor')
    }
    if (!this.tableName) {
      throw new Error('tableName must be provided in constructor')
    }
  }
