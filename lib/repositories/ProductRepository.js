...
class ProductRepository {
  ...
  async findByIds(productIds) {
    ...
    try {
      ...
      console.log('🔵 [ProductRepository] findByIds 시작:', productIds.length, '개')
      const { data, error } = await supabase
        .from('products')
        .select(`*`)
        .in('id', productIds)
      ...
      console.log('✅ [ProductRepository] findByIds 완료:', { elapsed: `${elapsed}ms`, count: data?.length })
      logger.debug('✅ [ProductRepository] 복수 상품 조회:', data?.length, '개')
      return data || []
    } catch (error) {
      const elapsed = Date.now() - startTime
      console.error('❌ [ProductRepository] findByIds 실패:', { elapsed: `${elapsed}ms`, error: error.message })
  ...
  /**
   * 일반 상품 재고 업데이트 (RPC - Lock 사용) ⭐ NEW!
   * Race Condition 방지: FOR UPDATE NOWAIT
   */
  async updateInventoryWithLock(productId, quantityChange) {
    try {
      const supabase = this._getClient()

      // RPC 함수 호출: Row-Level Lock으로 동시성 제어
      ...
      if (error) {
        ...
        if (error.message?.includes('Insufficient inventory')) {
          logger.warn('⚠️ [ProductRepository] 재고 부족:', productId, quantityChange)
          throw new Error('재고 부족')
        }

        // 2. Lock 타임아웃 에러 (다른 고객이 구매 중)
        if (error.message?.includes('concurrent_update') || error.code === '55P03') {
          logger.warn('⚠️ [ProductRepository] 동시 구매 감지:', productId)
      ...
      logger.info('✅ [ProductRepository] 재고 업데이트 (Lock):', productId, quantityChange, '→', data.new_inventory)
      return data
    } catch (error) {
      logger.error('❌ [ProductRepository] 재고 업데이트 실패 (Lock):', error)
      throw error
    }
  }
