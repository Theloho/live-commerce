...
export class CouponRepository extends BaseRepository {
  ...
  async incrementIssuedCount(couponId) {
    try {
      const { error } = await this.client.rpc('increment_coupon_issued_count', {
        coupon_id: couponId,
      })
      ...
      if (error) {
        ...
        if (coupon) {
          await this.update(couponId, {
            total_issued_count: (coupon.total_issued_count || 0) + 1,
          })
        }
      }
    } catch (error) {
  ...
  async incrementUsedCount(couponId) {
    try {
      const { error } = await this.client.rpc('increment_coupon_used_count', {
        coupon_id: couponId,
      })
      ...
      if (error) {
        ...
        if (coupon) {
          await this.update(couponId, {
            total_used_count: (coupon.total_used_count || 0) + 1,
          })
        }
      }
    } catch (error) {
