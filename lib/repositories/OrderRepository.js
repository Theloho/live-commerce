...
class OrderRepository {
  ...
  async findByUser(filters) {
    try {
      ...
      // status='all'인 경우 필터 적용 안 함 (모든 주문 조회)
  ...
  async findPendingOrdersWithGroup(filters) {
    try {
      ...
      logger.debug('🔍 [OrderRepository] verifying 주문 조회 시작:', { userId, kakaoId, excludeIds })
      ...
      let query = supabase
        ...
        `)
        .eq('status', 'verifying')  // ⭐ verifying만! (체크아웃 완료된 주문 = 합배 대상)
        .order('created_at', { ascending: false })
