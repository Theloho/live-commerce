  ...
  async hasPendingOrders(filters) {
    try {
      ...
      if (userId) {
        ...
        // ⚡ 성능 최적화: LIKE 제거 (Full table scan)
        // 카카오 사용자: user_id IS NULL + order_type 3가지 패턴 확인
        query = query
          .is('user_id', null)
          .or(`order_type.eq.direct:KAKAO:${kakaoId},order_type.eq.cart:KAKAO:${kakaoId},order_type.ilike.%KAKAO:${kakaoId}%`)
      }
