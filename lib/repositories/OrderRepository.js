...
class OrderRepository {
   ...
   * @param {Object} filters - { userId, kakaoId }
   * @returns {Promise<Object|null>} 주문 객체 또는 null
   */
  async findPendingOrdersWithGroup(filters) {
    try {
      const supabase = this._getClient()
      const { userId, kakaoId } = filters

      logger.debug('🔍 [OrderRepository] pending 주문 조회 시작:', { userId, kakaoId })

      let query = supabase
        .from('orders')
        .select('id, payment_group_id, created_at')
        .in('status', ['pending', 'verifying'])
        .order('created_at', { ascending: false })
        .limit(1)
