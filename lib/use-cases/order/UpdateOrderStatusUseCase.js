...
export class UpdateOrderStatusUseCase extends BaseUseCase {
  ...
  async execute({ orderIds, status, paymentData = null }) {
    try {
      ...
      // 2. ⭐ 합배 원칙: 일괄 처리 그룹 ID (기존 그룹 재사용 또는 신규 생성)
      const groupId = orderIds.length > 1
        ? await this._findOrReusePaymentGroupId(orderIds)
        : null
      if (groupId) {
   ...
   * @returns {Promise<string>} payment_group_id (GROUP-TIMESTAMP 형식)
   */
  async _findOrReusePaymentGroupId(orderIds) {
    try {
      this.log('🔍 [합배 원칙] payment_group_id 조회 시작', { orderIds })
      ...
      if (existingOrder?.payment_group_id) {
        this.log('✅ [합배 원칙] 기존 verifying 주문의 payment_group_id 재사용:', existingOrder.payment_group_id)
        return existingOrder.payment_group_id
      }

      // 4. 기존 그룹 없으면 신규 생성
      const newGroupId = `GROUP-${Date.now()}`
      this.log('🔵 [합배 원칙] 신규 payment_group_id 생성:', newGroupId)
      return newGroupId
    } catch (error) {
      this.log('⚠️ [합배 원칙] payment_group_id 조회 실패, 신규 생성:', error.message)
