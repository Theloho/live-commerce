...
export class UpdateOrderStatusUseCase extends BaseUseCase {
  ...
  async execute({ orderIds, status, paymentData = null }) {
    try {
      ...
      // 2. ⭐ 합배 원칙: 취소가 아닐 때만 그룹 ID 생성
      // ✅ 2025-11-13: 취소 시에는 payment_group_id 불필요
      let groupId = null
      if (status !== 'cancelled') {
        groupId = await this._findOrReusePaymentGroupId(orderIds, paymentData)
        if (groupId) {
  ...
  async _updateSingleOrder(orderId, status, paymentData, groupId, isRepresentative = true, groupOrderIndex = 0) {
    ...
    // ⭐ 취소 시에는 상태만 변경 (2025-11-13)
    if (status === 'cancelled') {
      await this.orderRepository.updateStatus(orderId, status)
      return
    }

    // 2. 일반 상태 변경 (verifying, paid, delivered 등)
    ...
    if (paymentData?.shippingData) {
      // ⭐ excludeOrderId 전달: 자기 자신은 합배 검사에서 제외 (2025-10-31 Bug #13 수정)
   ...
   * 3. 동일 배송지의 verifying 주문 존재 → ₩0 (합배 무료)
   * 4. shippingUtils.formatShippingInfo() 사용:
   *    - 제주 (63xxx): ₩4,000 + ₩3,000 = ₩7,000
   *    - 울릉도 (40200~40240): ₩4,000 + ₩5,000 = ₩9,000
   *    - 기타 도서산간 (59xxx 등): ₩4,000 + ₩5,000 = ₩9,000
   *    - 일반 지역: ₩4,000
   * @param {string} orderId - 주문 ID
   * @param {Object} shippingData - 배송 데이터
   * @param {number} groupOrderIndex - 그룹 내 순서 (0부터 시작)
   * @param {string|null} excludeOrderId - 합배 검사 시 제외할 주문 ID (자기 자신)
   */
  async _updateShipping(orderId, shippingData, groupOrderIndex = 0, excludeOrderId = null) {
    try {
      ...
      if (isFreeShipping) {
        ...
        // ⭐ excludeOrderId 전달: 자기 자신은 제외 (2025-10-31 Bug #13 수정)
   ...
   * @param {string|null} excludeOrderId - 제외할 주문 ID (자기 자신)
  ...
  async _findOrReusePaymentGroupId(orderIds, paymentData = null) {
    try {
      ...
      // 🔥 버그 수정 (2025-11-17): 동시 결제 시 중복 방지 위해 랜덤값 추가
      const randomSuffix = Math.random().toString(36).substring(2, 8) // 6자리 랜덤
      const newGroupId = `GROUP-${Date.now()}-${randomSuffix}`
      ...
      return orderIds.length > 1 ? `GROUP-${Date.now()}-${randomSuffix}` : null
    }
  }
