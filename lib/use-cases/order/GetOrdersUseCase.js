...
export class GetOrdersUseCase extends BaseUseCase {
  ...
  async execute({ user, orderId = null, page = 1, pageSize = 10, status = null }) {
    try {
      ...
      // ⚡ 1-3. 독립적인 DB 쿼리 병렬 실행 (성능 최적화: 50-60% 단축)
      const [statusCounts, orders, totalCount] = await Promise.all([
        this._fetchStatusCounts(user),                           // DB COUNT 쿼리
        this._fetchOrders(user, orderId, status, page, pageSize), // DB SELECT 쿼리
        this._fetchFilteredCount(user, status),                   // DB COUNT 쿼리
      ])

      // 4. 일괄결제 그룹 정보 계산 (orders에 의존하므로 순차 실행)
  ...
  _normalizeOrders(orders) {
    return orders.map((o) => {
      ...
      return {
        ...
        discount_amount: o.discount_amount || 0, // ✅ 하위 호환성 (주문 완료 페이지에서 사용)
  ...
  async _enrichBulkPaymentInfo(orders, user) {
    ...
    try {
      ...
      const groupIds = [...new Set(orders.filter(o => o && o.payment_group_id).map(o => o.payment_group_id))]
      ...
      // ⚡ 각 그룹의 모든 주문 조회 (병렬 실행으로 N+1 문제 해결)
      const groupOrdersMap = {}

      // Promise.all()로 모든 그룹 조회를 병렬 실행
      const groupOrdersPromises = groupIds.map(groupId =>
        this.orderRepository.findByPaymentGroup({
          userId: user.kakao_id ? null : user.id,
          kakaoId: user.kakao_id || null,
          paymentGroupId: groupId
        }).catch(error => {
          // 개별 그룹 조회 실패해도 다음 그룹 계속 처리
          console.warn(`⚠️ 그룹 ${groupId} 조회 실패:`, error.message)
          return { orders: [] }
        })
      )

      const groupOrdersResults = await Promise.all(groupOrdersPromises)

      // 결과를 Map으로 변환
      groupIds.forEach((groupId, index) => {
        groupOrdersMap[groupId] = groupOrdersResults[index]?.orders || []
      })
      ...
      return orders.map(order => {
        ...
        try {
          ...
          // ⭐ 대표 주문: group_order_index = 0 (우선), fallback: created_at 기준
          ...
          // ⭐ 일괄결제: 쿠폰 1개만 적용 (대표 주문의 discount_amount)
          ...
          // 배송비 (대표 주문만, order_shipping에서 조회)
          ...
          return {
            ...
            bulkPaymentInfo: {
              ...
              groupTotalAmount: totalAmountSum - groupTotalDiscount + groupShippingFee  // 총 입금금액 (배송비 포함)
            }
          }
        } catch (mapError) {
          // 개별 주문 처리 실패 시 원본 반환
  ...
  async getById({ orderId, user = null }) {
    try {
      ...
      // ⭐ 일괄결제 그룹 정보 계산 (주문 목록과 동일한 로직)
      let orderWithGroupInfo = [order]
      if (user) {
        try {
          orderWithGroupInfo = await this._enrichBulkPaymentInfo([order], user)
        } catch (enrichError) {
          // 그룹 정보 계산 실패해도 주문은 반환 (그룹 정보 없이)
          console.warn('⚠️ 일괄결제 그룹 정보 계산 실패 (무시):', enrichError.message)
        }
      }

      // 데이터 정규화
      const normalized = this._normalizeOrders(orderWithGroupInfo)[0]
