...
export class CreateOrderUseCase extends BaseUseCase {
  ...
  /**
   * 주문 생성 실행
   * @param {Object} params - { orderData, shipping, payment, coupon?, user, isQuickPurchase? }
   * @param {boolean} params.isQuickPurchase - BuyBottomSheet 빠른 구매 모드 (무료배송/합배 체크 스킵)
   * @returns {Promise<Order>} 생성된 Order Entity
   * @throws {ValidationError} 검증 실패 시
   * @throws {InsufficientInventoryError} 재고 부족 시
   */
  async execute({ orderData, shipping, payment, coupon = null, user, isQuickPurchase = false }) {
    const timings = {}
    ...
    try {
      ...
      // 1.5. 무료배송 확인 (⚡ isQuickPurchase 시 스킵 - pending 주문은 체크아웃에서 설정)
      const t2 = Date.now()
      ...
      // 1.6. ⭐ 합배 원칙: 기존 pending/verifying 주문의 payment_group_id 조회 (⚡ isQuickPurchase 시 스킵)
      const t2_5 = Date.now()
      let paymentGroupId = null
      if (!isQuickPurchase) {
        paymentGroupId = await this._findExistingPaymentGroup(user, shipping)
      ...
      // 3. 금액 계산
      const t4 = Date.now()
      // ✅ pending 주문은 배송비 0 (일괄결제 시 UpdateOrderStatusUseCase에서 재계산)
      // 이유: 장바구니(pending)는 결제 전이므로 배송비 미확정
      // 실제 배송비는 체크아웃(verifying 전환) 시 확정됨
      const shippingFee = 0  // ⭐ 2025-10-30: total_amount 배송비 중복 방지
      ...
      // 4. 재고 차감 (Lock으로 확인+차감 동시에 - 중복 제거)
      const t5 = Date.now()
      this.log('🔵 [CreateOrderUseCase] 재고 차감 시작 (Lock)')
      await this._deductInventory(orderData.items)
      timings.inventoryDeduction = Date.now() - t5
      ...
      // 6. 상품 정보 조회 (⚡ isQuickPurchase 시 스킵 - orderData에 이미 포함)
      const t6 = Date.now()
      let productMap = new Map()

      if (!isQuickPurchase) {
        // 체크아웃: DB에서 최신 상품 정보 조회
        this.log('🔵 [CreateOrderUseCase] 상품 정보 조회 시작')
        const productIds = orderData.items.map(item => item.product_id)
        const products = await this.productRepository.findByIds(productIds)
        productMap = new Map(products.map(p => [p.id, p]))
        this.log('✅ [CreateOrderUseCase] 상품 정보 조회 완료:', `${Date.now() - t6}ms`)
      } else {
        // BuyBottomSheet: orderData에서 직접 사용 (이미 product_number, thumbnail_url 포함)
        productMap = new Map(
