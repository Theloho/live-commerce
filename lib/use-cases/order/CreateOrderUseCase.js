...
export class CreateOrderUseCase extends BaseUseCase {
  ...
  async execute({ orderData, shipping, payment, coupon = null, user }) {
    ...
    try {
      ...
      const t2_5 = Date.now()
      const paymentGroupId = await this._findExistingPaymentGroup(user)
   ...
   */
  async _findExistingPaymentGroup(user) {
    try {
      ...
      // 기존 주문이 이미 payment_group_id를 가지고 있으면 재사용
      if (existingOrder.payment_group_id) {
        this.log('✅ [합배 원칙] 기존 payment_group_id 재사용:', existingOrder.payment_group_id)
        return existingOrder.payment_group_id
      }

      // 기존 주문이 payment_group_id가 없으면 신규 생성 후 기존 주문에도 적용
      const newGroupId = crypto.randomUUID()
      this.log('🔵 [합배 원칙] 신규 payment_group_id 생성:', newGroupId)

      // 기존 주문에 payment_group_id 업데이트
