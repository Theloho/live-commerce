...
export class DistributeCouponUseCase extends BaseUseCase {
  ...
  async _filterDuplicates(couponId, userIds) {
    try {
      // ✅ 2025-11-03 Bug #17-3: getCouponHolderIds() 사용 (profiles JOIN 제거)
      // - profiles 삭제되어도 user_coupons는 남아있음
      // - JOIN 실패로 인한 중복 체크 누락 방지
      const holderIds = await this.couponRepository.getCouponHolderIds(couponId)
      const holderSet = new Set(holderIds)

      const newUsers = userIds.filter((userId) => !holderSet.has(userId))
      const alreadyHasUsers = userIds.filter((userId) => holderSet.has(userId))
