...
class CreateOrderUseCase {
  ...
  async _createItem(oid, od) {
    const { error } = await this._db().from('order_items').insert([{ order_id: oid, product_id: od.id, title: od.title, quantity: od.quantity, price: od.price, total: od.totalPrice, unit_price: od.price, total_price: od.totalPrice, selected_options: od.selectedOptions || {}, variant_title: od.variant || null, variant_id: od.variantId || null, sku: od.sku || null, product_snapshot: od.productSnapshot || {} }])
