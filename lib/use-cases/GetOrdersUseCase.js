...
class GetOrdersUseCase {
  ...
  async _fetchOrders(user, orderId, status = null, limit = null, offset = null) {
    ...
    const baseQuery = `
      ...
      order_items!inner (
        id, order_id, product_id, variant_id, title, price, unit_price,
        quantity, total, total_price, thumbnail_url, product_number, selected_options
      ),
      order_shipping (
        id, order_id, recipient_name, phone, address, detail_address,
        postal_code, shipping_fee, shipping_message
      ),
      order_payments (
