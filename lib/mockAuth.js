...
function loadMockProducts() {
  ...
  try {
    ...
    if (saved && saved !== 'null') {
      ...
      // 데이터 유효성 검사: title이 없으면 손상된 데이터
      if (products.length > 0 && !products[0].title) {
        console.warn('localStorage 상품 데이터가 손상됨, 기본 상품으로 재초기화')
        const freshProducts = [...defaultProducts]
        localStorage.setItem('mock_products', JSON.stringify(freshProducts))
...
export const updateProductInventory = (productId, quantityChange) => {
  ...
  try {
    ...
    // 재고 정보만 localStorage에 저장 (용량 최적화)
    const inventoryData = currentProducts.map(product => ({
      id: product.id,
      inventory_quantity: product.inventory_quantity
    }))
    console.log('저장할 재고 데이터:', inventoryData)

    localStorage.setItem('mock_products', JSON.stringify(inventoryData))
    console.log('localStorage에 재고 데이터 저장 완료')
