  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // ⭐ 헤더가 2행에 있는 경우 감지 및 처리
        const firstRow = jsonData[0]
        const firstRowKeys = Object.keys(firstRow)

        // 첫 번째 행의 값들을 확인
        const firstRowValues = Object.values(firstRow)
        console.log('📋 첫 번째 행:', firstRowValues)

        // 첫 번째 행이 제목이면 (예: "송장번호_업로드_샘플" 같은 텍스트)
        const isFirstRowTitle = firstRowValues.some(val =>
          val && String(val).includes('샘플') || String(val).includes('업로드') || String(val).includes('제목')
        )

        if (isFirstRowTitle) {
          console.log('⚠️ 첫 번째 행이 제목입니다. 2행부터 읽기 시작...')
          // 2행부터 다시 읽기 (range 옵션 사용)
        ...
        console.log('📊 총 데이터 행:', jsonData.length)
