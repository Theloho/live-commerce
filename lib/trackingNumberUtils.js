...
export function getTrackingUrl(carrier, trackingNumber) {
  ...
  // 택배사별 직접 추적 URL
  const trackingUrls = {
    cj: `https://www.cjlogistics.com/ko/tool/parcel/tracking?paramInvcNo=${trackingNumber}`,
    ...
    epost: `https://service.epost.go.kr/trace.RetrieveDomRigiTraceList.comm?sid1=${trackingNumber}`,
    // 기타 택배사는 스마트택배 통합 조회로 연결
    default: `https://www.smartparcel.kr/tracking?t_key=${trackingNumber}`
  }

  // 해당 택배사 URL 반환, 없으면 스마트택배 기본 URL
  return trackingUrls[carrier] || trackingUrls.default
}
