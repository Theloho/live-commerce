...
export class OrderCalculations {
   ...
   * @param {number} itemsTotal - 상품 총액 (미사용, 향후 확장용)
   * @param {string} postalCodeOrRegion - 우편번호(5자리) 또는 지역명 ('제주', '울릉도/독도', 'normal' 등)
   * @param {number} baseShippingFee - 기본 배송비 (무료배송 조건 적용 시 0 전달)
   * @returns {number} 배송비
   */
  static calculateShippingFee(itemsTotal, postalCodeOrRegion = 'normal', baseShippingFee = 4000) {
    // ✅ 우편번호인 경우 (5자리 숫자) - shippingUtils 사용
    if (/^\d{5}$/.test(postalCodeOrRegion)) {
      const { formatShippingInfo } = require('./shippingUtils')
      const shippingInfo = formatShippingInfo(baseShippingFee, postalCodeOrRegion)  // ← 전달받은 기본 배송비 사용
      if (isDevelopment) console.log(`🚚 배송비 (우편번호 ${postalCodeOrRegion}, ${shippingInfo.region || '일반'}): 기본 ₩${shippingInfo.baseShipping.toLocaleString()} + 추가 ₩${shippingInfo.surcharge.toLocaleString()} = ₩${shippingInfo.totalShipping.toLocaleString()}`)
      return shippingInfo.totalShipping
    }

    // ✅ 지역명인 경우 ('제주', '울릉도/독도', '도서산간') - 우편번호로 변환 후 계산
    if (postalCodeOrRegion === '제주') {
      const { formatShippingInfo } = require('./shippingUtils')
      const shippingInfo = formatShippingInfo(baseShippingFee, '63000')  // 제주 대표 우편번호
      if (isDevelopment) console.log(`🚚 배송비 (제주): 기본 ₩${shippingInfo.baseShipping.toLocaleString()} + 추가 ₩${shippingInfo.surcharge.toLocaleString()} = ₩${shippingInfo.totalShipping.toLocaleString()}`)
      return shippingInfo.totalShipping
    } else if (postalCodeOrRegion === '울릉도/독도') {
      const { formatShippingInfo } = require('./shippingUtils')
      const shippingInfo = formatShippingInfo(baseShippingFee, '40200')  // 울릉도 대표 우편번호
      if (isDevelopment) console.log(`🚚 배송비 (울릉도/독도): 기본 ₩${shippingInfo.baseShipping.toLocaleString()} + 추가 ₩${shippingInfo.surcharge.toLocaleString()} = ₩${shippingInfo.totalShipping.toLocaleString()}`)
      return shippingInfo.totalShipping
    } else if (postalCodeOrRegion === '도서산간') {
      // 기타 도서산간 지역 (+5000원)
      const surcharge = baseShippingFee > 0 ? 5000 : 0  // 무료배송이면 추가비도 무료
      const totalShippingFee = baseShippingFee + surcharge
      if (isDevelopment) console.log(`🚚 배송비 (도서산간): 기본 ₩${baseShippingFee.toLocaleString()} + 추가 ₩${surcharge.toLocaleString()} = ₩${totalShippingFee.toLocaleString()}`)
      return totalShippingFee
    }

    // ✅ 레거시 호환: 'normal', 'remote', 'island' 등
    const regionFees = {
      normal: 0,      // 일반 지역
      remote: 5000,   // 도서산간 (추가 5000원)
      island: 5000    // 특수 지역 (추가 5000원)
    }

    // 무료배송이면 추가비도 무료
    const surcharge = baseShippingFee > 0 ? (regionFees[postalCodeOrRegion] || 0) : 0
    const totalShippingFee = baseShippingFee + surcharge
    if (isDevelopment) console.log(`🚚 배송비 (레거시: ${postalCodeOrRegion}): ₩${totalShippingFee.toLocaleString()}`)
   ...
   * @param {array} items - 주문 아이템 배열
   * @param {object} options - 계산 옵션 { region?: string, coupon?: object, paymentMethod?: string, baseShippingFee?: number }
   * @returns {object} 완전한 주문 계산 결과
   */
  static calculateFinalOrderAmount(items, options = {}) {
    const { region = 'normal', coupon = null, paymentMethod = 'transfer', baseShippingFee = 4000 } = options
    ...
    // 3. 배송비 계산 (무료배송 조건 적용) + breakdown 추출
    const shippingFee = this.calculateShippingFee(itemsTotal, region, baseShippingFee)

    // 3-1. breakdown 정보 추출 (우편번호인 경우에만)
    let shippingBreakdown = null
    if (/^\d{5}$/.test(region)) {
      const { formatShippingInfo } = require('./shippingUtils')
      const shippingInfo = formatShippingInfo(baseShippingFee, region)
      shippingBreakdown = {
    ...
    const result = {
      ...
      shippingBreakdown: shippingBreakdown, // ⭐ 배송비 breakdown 추가
