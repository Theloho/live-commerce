...
export class OrderCalculations {
   ...
   * @returns {number} 배송비
   */
  static calculateShippingFee(itemsTotal, postalCodeOrRegion = 'normal') {
    // ✅ 우편번호인 경우 (5자리 숫자) - shippingUtils 사용
    if (/^\d{5}$/.test(postalCodeOrRegion)) {
      const { formatShippingInfo } = require('./shippingUtils')
      const shippingInfo = formatShippingInfo(4000, postalCodeOrRegion)  // ← 기본 배송비 4000원
    ...
    if (postalCodeOrRegion === '제주') {
      const { formatShippingInfo } = require('./shippingUtils')
      const shippingInfo = formatShippingInfo(4000, '63000')  // 제주 대표 우편번호
      ...
      const { formatShippingInfo } = require('./shippingUtils')
      const shippingInfo = formatShippingInfo(4000, '40200')  // 울릉도 대표 우편번호
    ...
    const baseShippingFee = 4000  // ← 기본 배송비 4000원 (원복)
    const regionFees = {
      normal: 0,      // 일반 지역
    ...
    const totalShippingFee = baseShippingFee + (regionFees[postalCodeOrRegion] || 0)
