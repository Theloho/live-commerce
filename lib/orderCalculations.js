...
export class OrderCalculations {
  ...
  static applyCouponDiscount(itemsTotal, coupon = null) {
    ...
    if (coupon.type === 'percentage') {
      ...
      if (coupon.maxDiscount && discountAmount > coupon.maxDiscount) {
        ...
        console.log(`🎟️ 퍼센트 할인 (최대 제한 적용): ₩${itemsTotal.toLocaleString()} × ${coupon.value}% = ₩${discountAmount.toLocaleString()} (최대 ₩${coupon.maxDiscount.toLocaleString()})`)
      } else {
        console.log(`🎟️ 퍼센트 할인: ₩${itemsTotal.toLocaleString()} × ${coupon.value}% = ₩${discountAmount.toLocaleString()}`)
      }
    } else if (coupon.type === 'fixed_amount') {
      // 금액 할인: 쿠폰 금액과 상품 금액 중 작은 값
      discountAmount = Math.min(coupon.value, itemsTotal)
      console.log(`🎟️ 금액 할인: MIN(₩${coupon.value.toLocaleString()}, ₩${itemsTotal.toLocaleString()}) = ₩${discountAmount.toLocaleString()}`)
    }
  ...
  static calculateFinalOrderAmount(items, options = {}) {
    ...
    console.log('🧮 최종 주문 계산 완료:', result.breakdown)
    return result
  }
  ...
  static applyDiscount(baseAmount, discount = null) {
    console.warn('⚠️ applyDiscount는 deprecated입니다. calculateFinalOrderAmount 사용을 권장합니다.')
