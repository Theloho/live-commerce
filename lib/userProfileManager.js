...
export class UserProfileManager {
      ...
      if (typeof window !== 'undefined' && sessionStorage) {
        try {
          ...
          if (storedUser) {
            ...
            console.log('✅ sessionStorage 사용자 확인 (카카오):', sessionUser.id)
            return sessionUser
          }
        } catch (storageError) {
          console.warn('⚠️ sessionStorage 읽기 실패:', storageError)
        }
      }

      // 4. 둘 다 없으면 null 반환
      console.log('ℹ️ 활성 세션 없음 (Auth + sessionStorage 모두 없음)')
      return null
  ...
  static async loadUserProfile(userId) {
    try {
      ...
      console.log('✅ loadUserProfile 성공:', userId)
      return profile
    } catch (error) {
