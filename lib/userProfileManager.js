...
export class UserProfileManager {
  ...
  static async getCurrentUser() {
    try {
      ...
      if (session?.user) {
        // ⚡ authStore 캐시 확인 (profiles 조회 전)
        // ❌ setUser() 제거: useAuth hook이 이미 세션 모니터링 중 (무한 루프 방지)
        const cachedProfile = useAuthStore.getState().profile
        if (cachedProfile && cachedProfile.id === session.user.id) {
          console.log('⚡ [CACHE HIT] 프로필 캐시 사용 (DB 쿼리 생략)')
          return cachedProfile
        }

        // 3. profiles 테이블에서 사용자 정보 조회 (캐시 미스 시에만)
        const { data: profile, error: profileError } = await supabase
          .from('profiles')
          .select('*')
          .eq('id', session.user.id)
          .maybeSingle() // ⚡ single() → maybeSingle() (0 rows 시 error 없음)

        if (profileError) {
          console.warn('⚠️ 프로필 조회 실패, user_metadata 사용:', profileError)
        ...
        if (!profile) {
          console.warn('⚠️ 프로필 레코드 없음, user_metadata 사용')
          const fallbackUser = {
            id: session.user.id,
            email: session.user.email,
            ...session.user.user_metadata
  ...
  /**
   * 특정 사용자 ID로 프로필 정보 조회 (캐시 우선)
   * @param {string} userId - 사용자 ID
   * @param {boolean} forceRefresh - 캐시 무시하고 DB에서 강제 조회
  ...
  static async loadUserProfile(userId, forceRefresh = false) {
    try {
      ...
      if (!forceRefresh) {
        ...
        // 캐시된 프로필이 있고, 동일한 사용자라면 즉시 반환 (DB 쿼리 생략!)
        if (cachedProfile && cachedProfile.id === userId) {
          console.log('⚡ 캐시에서 프로필 로드 (DB 쿼리 생략):', userId)
          return cachedProfile
        }
      }

      // ⚡ 2. DB 조회 (캐시 미스 또는 강제 갱신)
      console.log('🔍 DB에서 프로필 조회:', userId)
      const { data: profile, error } = await supabase
        .from('profiles')
        .select('*')
        .eq('id', userId)
        .maybeSingle() // ⚡ single() → maybeSingle() (신규 가입 직후 0 rows 허용)
      ...
      if (!profile) {
        console.warn('⚠️ 프로필 레코드 없음 (신규 가입 직후일 수 있음):', userId)
        return null
      }
      ...
      if (profile) {
        useAuthStore.getState().setProfile(profile)
        console.log('✅ 프로필 캐시 저장 완료:', userId)
      }
