...
export const getAllOrders = async () => {
  try {
    ...
    const ordersWithItems = await Promise.all(data.map(async order => {
      ...
      // 🔍 디버깅: shipping 정보 확인
      console.log('📦 주문 shipping 정보:', {
        order_id: order.id,
        payment_group_id: order.payment_group_id,
        shipping_data: shipping,
        has_phone: !!shipping?.phone,
        phone_value: shipping?.phone
      })
...
export const updateOrderStatus = async (orderId, status, paymentData = null) => {
  try {
    ...
    if (status === 'verifying') {
      ...
      console.log('🕐 결제 확인중 시간 기록:', now)
    }
    if (status === 'paid') {
      updateData.paid_at = now
      console.log('💰 결제 완료 시간 기록:', now)
    }
    if (status === 'delivered') {
      updateData.delivered_at = now
      console.log('🚚 발송 완료 시간 기록:', now)
    }
    if (status === 'cancelled') {
      updateData.cancelled_at = now
      console.log('❌ 주문 취소 시간 기록:', now)
    }
    ...
    console.log('✅ 주문 상태 업데이트 완료:', { orderId, status, timestamp: now })
    return true
  } catch (error) {
...
export const updateMultipleOrderStatus = async (orderIds, status, paymentData = null) => {
  try {
    ...
    for (const orderId of orderIds) {
      ...
      if (status === 'verifying') {
        updateData.verifying_at = now
        console.log('🕐 결제 확인중 시간 기록 (일괄):', now)
      }
      if (status === 'paid') {
        updateData.paid_at = now
        console.log('💰 결제 완료 시간 기록 (일괄):', now)
      }
      if (status === 'delivered') {
        updateData.delivered_at = now
        console.log('🚚 발송 완료 시간 기록 (일괄):', now)
      }
      if (status === 'cancelled') {
        updateData.cancelled_at = now
        console.log('❌ 주문 취소 시간 기록 (일괄):', now)
      }
      ...
      if (paymentData && paymentData.shippingData) {
        ...
        console.log('📦 orders 테이블 업데이트 데이터:', updateData)
      }
      ...
      if (paymentData && paymentData.shippingData) {
        const shippingData = paymentData.shippingData
        console.log('📦 order_shipping 테이블 업데이트 시작:', orderId)
        ...
        console.log('📦 order_shipping PATCH 요청:', shippingUpdatePayload)
        ...
        if (!shippingUpdateResponse.ok) {
          console.warn(`⚠️ order_shipping UPDATE 실패 (${orderId}), INSERT 시도...`)
          ...
          if (!shippingInsertResponse.ok) {
            ...
            console.log(`✅ order_shipping INSERT 성공 (${orderId})`)
          }
        } else {
          console.log(`✅ order_shipping UPDATE 성공 (${orderId}) - phone: ${shippingData.shipping_phone}`)
        }
      }
