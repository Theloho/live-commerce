...
export const getOrders = async (userId = null) => {
  try {
    ...
    if (userQuery.type === 'kakao') {
      ...
      query = query.eq(userQuery.query.column, userQuery.query.value)
  ...
  try {
    ...
    const ordersWithItems = await Promise.all(data.map(async order => {
      ...
      if (order.user_id) {
        ...
        // 카카오 주문인지 확인
        if (order.order_type?.startsWith('direct:KAKAO:')) {
          const kakaoId = order.order_type.replace('direct:KAKAO:', '')
