  ...
  try {
    ...
    for (const orderId of orderIds) {
      ...
      if (paymentData?.shippingData) {
        const { shipping_name, shipping_phone, shipping_address, shipping_detail_address } = paymentData.shippingData
        if (shipping_name) updateData.shipping_name = shipping_name
        if (shipping_phone) updateData.shipping_phone = shipping_phone
        if (shipping_address) updateData.shipping_address = shipping_address
        if (shipping_detail_address !== undefined) updateData.shipping_detail_address = shipping_detail_address

        console.log('📦 배송지 정보 업데이트:', paymentData.shippingData)
      }
