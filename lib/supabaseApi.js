...
export const createOrder = async (orderData, userProfile, depositName = null) => {
  try {
    ...
    const orderData_final = {
      ...
      status: 'pending',
      order_type: user.provider === 'kakao'
        ? `${orderData.orderType || 'direct'}:KAKAO:${user.id}`
    ...
    const itemData = {
      ...
      total_price: normalizedOrderData.totalPrice,
    ...
    const { error: shippingError } = await supabase
      ...
      .insert([{
        ...
        address: userProfile.address, // 이미 selectedAddress가 반영된 상태
...
export const getOrders = async (userId = null) => {
  try {
    // UserProfileManager를 활용한 통합 사용자 정보 가져오기
    ...
    if (userId) {
      ...
      if (!rawUser) throw new Error('로그인이 필요합니다')

      // UserProfileManager로 프로필 정규화
      const normalizedProfile = UserProfileManager.normalizeProfile(rawUser)
      user = {
        id: rawUser.id,
        name: normalizedProfile.name || rawUser.name,
        email: rawUser.email,
        ...normalizedProfile
      }
    }
    ...
    console.log('📊 정규화된 사용자 정보:', user)

    // 🔍 DB 실제 데이터 조사: 현재 사용자 ID로 주문이 정말 없는지 확인
    console.log('🔍 DB 조사 1단계: 현재 사용자 ID로 주문 검색')

    const userOrdersCheck = await supabase
      .from('orders')
      .select('id, user_id, customer_order_number, created_at')
      .eq('user_id', user.id)
      .neq('status', 'cancelled')

    console.log(`🔍 사용자 ID ${user.id}로 조회된 주문:`, userOrdersCheck.data?.length || 0, '개')

    // 🔍 DB 조사 2단계: 모든 pending 주문의 user_id 확인
    console.log('🔍 DB 조사 2단계: 모든 pending 주문 user_id 확인')

    const allPendingOrders = await supabase
      .from('orders')
      .select('id, user_id, customer_order_number, order_shipping(name)')
      .eq('status', 'pending')
      .order('created_at', { ascending: false })

    console.log('🔍 모든 pending 주문들:', allPendingOrders.data?.map(o => ({
      id: o.id,
      user_id: o.user_id,
      order_number: o.customer_order_number,
      shipping_name: o.order_shipping?.[0]?.name
    })))

    // 이제 정확한 필터링으로 사용자 주문 가져오기
    const { data, error } = await supabase
      .from('orders')
      .select(`
        *,
    ...
    // 🔍 정확한 사용자별 필터링: user_id 기반으로만 처리
    // DB에서 이미 user_id로 필터링되었으므로 추가 검증 불필요
    console.log('✅ 정확한 필터링: user_id 기반 주문만 허용')
