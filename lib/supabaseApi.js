...
export const getAllOrders = async () => {
  try {
    ...
    const ordersWithItems = data.map(order => ({
      ...
      userName: order.users?.name || '알 수 없음',
      userNickname: order.users?.nickname || '알 수 없음',
      userEmail: order.users?.email || '알 수 없음'
    }))
...
export const getAllCustomers = async () => {
  try {
    ...
    // 고객별 주문 통계 계산
    const customersWithStats = data.map(user => ({
      ...user,
      totalOrders: user.orders.length,
      totalAmount: user.orders.reduce((sum, order) => sum + (order.total_amount || 0), 0),
      lastOrderDate: user.orders.length > 0 ? user.orders[0].created_at : null,
      recentOrders: user.orders.slice(0, 3) // 최근 3개 주문만
    }))
