  ...
  try {
    ...
    if (orderData.orderType === 'cart') {
      ...
      if (pendingOrders && pendingOrders.length > 0) {
        ...
        logger.debug('✅ 기존 장바구니 주문 발견 (옵션), 아이템 추가:', orderId)
      }
    }
    ...
    if (existingOrder) {
      ...
      order = updatedOrder
      logger.debug('✅ 주문 금액 업데이트 (옵션):', order.total_amount)
    } else {
...
export const createOrder = async (orderData, userProfile, depositName = null) => {
  try {
    ...
    // 1. 장바구니 주문 병합: 기존 pending 주문 찾기
    let orderId = null
    let customerOrderNumber = null
    let existingOrder = null

    // orderType이 'cart'인 경우 기존 pending 주문 검색
    if (orderData.orderType === 'cart') {
      const orderTypePattern = user.kakao_id
        ? `cart:KAKAO:${user.kakao_id}`
        : 'cart'
      ...
      if (pendingOrders && pendingOrders.length > 0) {
        ...
        orderId = existingOrder.id
        customerOrderNumber = existingOrder.customer_order_number
        logger.debug('✅ 기존 장바구니 주문 발견, 아이템 추가:', orderId)
      }
    }
    ...
    if (existingOrder) {
      ...
      order = updatedOrder
      logger.debug('✅ 주문 금액 업데이트:', order.total_amount)
    } else {
      // 새 주문 생성
      const orderData_final = {
        id: orderId,
      ...
      console.log('💾 DB INSERT orders:', {
        ...
        source_couponDiscount: orderData.couponDiscount
      })
    ...
    if (!existingOrder) {
      ...
      logger.debug('💰 장바구니 주문 결제 금액 업데이트:', {
      ...
      logger.debug('✅ 결제 금액 업데이트 완료:', newPaymentAmount)
    }
...
export const getOrders = async (userId = null) => {
  try {
    ...
    // 통합 사용자 식별 정보 가져오기
    const userQuery = await UserProfileManager.getUserOrderQuery()
    ...
    const ordersWithItems = finalData.map(order => ({
      ...
      shipping: {
        ...
        postal_code: order.order_shipping[0]?.postal_code || null // ⭐ 배송비 계산용
      },
...
export const updateMultipleOrderStatus = async (orderIds, status, paymentData = null) => {
  try {
    console.log('🔵 [1] updateMultipleOrderStatus 함수 시작')
    console.log('🔵 [1] 전달받은 파라미터:', {
      orderIds,
      status,
      paymentData_exists: !!paymentData,
      paymentData_discountAmount: paymentData?.discountAmount,
      paymentData_shippingData_postal_code: paymentData?.shippingData?.shipping_postal_code
    })
