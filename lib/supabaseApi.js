...
export const getOrderById = async (orderId) => {
  try {
    ...
    const result = {
      ...
      // 사용자 정보 추가
      userId: data.user_id,
      userName: userInfo?.name || userInfo?.user_metadata?.name || data.order_shipping?.[0]?.recipient_name || '정보없음',
    ...
    console.log('🎯 최종 주문 상세 결과:', {
      ...
      shipping_name: result.shipping?.recipient_name,
      shipping_phone: result.shipping?.recipient_phone,
