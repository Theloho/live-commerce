  ...
  try {
    ...
    const ordersWithItems = finalData.map(order => ({
      ...
      items: order.order_items.map(item => ({
        ...
        title: item.title || '상품', // 새로운 스키마 title 컬럼 사용
        quantity: item.quantity,
        price: item.price || 0, // 새로운 스키마 price 컬럼 사용
        total: item.total || (item.price * item.quantity) || 0, // 새로운 스키마 total 컬럼 사용
        totalPrice: item.total || (item.price * item.quantity) || 0, // UI 호환성을 위한 alias
      ...
      if (order.payment_group_id && !processedGroupIds.has(order.payment_group_id)) {
        ...
        if (groupOrders.length > 1) {
          ...
          const groupOrder = {
            ...
            payment: (() => {
              // 상품 금액만 합산 - 새로운 스키마 필드 사용
              const itemsTotal = groupOrders.reduce((sum, o) => {
                return sum + (o.items || []).reduce((itemSum, item) => {
                  return itemSum + (item.total || (item.price * item.quantity))
                }, 0)
              }, 0)
              // 배송비는 한 번만 추가
              const shippingFee = 4000
            ...
            order_payments: [(() => {
              ...
              console.log('🔧 그룹 주문 payment.amount 계산 디버깅:', {
                ...
                groupOrders_items: groupOrders.map(o => ({
                  ...
                  items_total: (o.items || []).reduce((sum, item) => sum + (item.total || (item.price * item.quantity)), 0)
                }))
              })
              return {
                ...order.payment,
