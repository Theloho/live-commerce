...
export const getAllOrders = async () => {
  try {
    ...
    const ordersWithItems = data.map(order => {
      ...
      // 사용자 정보 - shipping 테이블에서 실제 정보 사용
      const userName = shipping.name || '정보없음'
      const userPhone = shipping.phone || '정보없음'
      const userNickname = userName !== '정보없음' ? userName : '익명'
      ...
      return {
        ...
        userName,
        userNickname,
        userEmail: order.user_id ? `user-${order.user_id.slice(-8)}` : '정보없음'
      }
    })

    console.log(`🎯 최종 반환될 주문 수: ${ordersWithItems.length}`)
    ...
    orders.forEach(order => {
      ...
      if (!customerMap.has(userId)) {
        customerMap.set(userId, {
          id: userId,
          name: shippingInfo.recipient_name || '정보없음',
          nickname: shippingInfo.recipient_name || 'Unknown',
          phone: shippingInfo.recipient_phone || '정보없음',
          address: shippingInfo.address || '정보없음',
          tiktokId: '',
          youtubeId: '',
          kakaoLink: '',
          created_at: order.created_at,
...
export const getOrderById = async (orderId) => {
  try {
    ...
    const result = {
      ...
      // 사용자 정보 추가
      userId: data.user_id,
      userName: userInfo?.name || userInfo?.user_metadata?.name || data.order_shipping?.[0]?.name || '정보없음',
      userNickname: userInfo?.nickname || userInfo?.user_metadata?.nickname || (data.order_shipping?.[0]?.name !== '정보없음' ? data.order_shipping?.[0]?.name : '익명'),
    ...
    for (const orderId of orderIds) {
      ...
      if (paymentData) {
        ...
        if (!paymentUpdateResponse.ok) {
          ...
          console.log(`✅ 결제 테이블 UPDATE 성공 (${orderId})`)
        }
      }
...
export const getUserById = async (userId) => {
  try {
    ...
    if (error || !user || user.id !== userId) {
      // auth에서 찾을 수 없으면 profiles 테이블에서 조회
      ...
      if (profileError) {
        console.log('프로필 테이블에서 사용자 정보를 찾을 수 없음:', userId)
        return null
      }
