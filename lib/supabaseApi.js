...
export const getAllOrders = async () => {
  try {
    ...
    const ordersWithItems = await Promise.all(data.map(async order => {
      ...
      const userNickname = userInfo?.user_metadata?.nickname || userInfo?.nickname || (shipping.name !== '정보없음' ? shipping.name : '익명')

      // 입금자명: 계좌이체의 경우 사용자명을 기본값으로 사용 (나중에 사용자가 설정 가능)
      const depositName = userName !== '정보없음' ? userName : null
      ...
      if (order.payment_group_id && !processedGroupIds.has(order.payment_group_id)) {
        ...
        if (groupOrders.length > 1) {
          ...
          const groupOrder = {
            ...
            payment: {
              ...order.payment,
              amount: groupOrders.reduce((total, o) => total + (o.payment?.amount || 0), 0)
            },
            // 첫 번째 주문의 사용자 정보 사용
            userId: order.userId,
...
export const getOrderById = async (orderId) => {
  try {
    ...
    const userNickname = userInfo?.nickname || userInfo?.user_metadata?.nickname || (data.order_shipping?.[0]?.name !== '정보없음' ? data.order_shipping?.[0]?.name : '익명')

    // 입금자명: 계좌이체의 경우 사용자명을 기본값으로 사용
    const depositName = userName !== '정보없음' ? userName : null
