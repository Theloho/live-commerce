...
export const createOrder = async (orderData, userProfile, depositName = null) => {
  try {
    ...
    // 2. 주문 아이템 생성 (실제 DB 스키마: unit_price, total_price, title 컬럼 없음)
    const itemData = {
      order_id: orderId,
      product_id: normalizedOrderData.id,
      quantity: normalizedOrderData.quantity,
      unit_price: normalizedOrderData.price, // 실제 DB는 unit_price 사용
  ...
  try {
    ...
    const ordersWithItems = finalData.map(order => ({
      ...
      items: order.order_items.map(item => ({
        ...
        title: item.products?.title || '상품', // 실제 DB에는 title 컬럼 없음, products 테이블에서 가져옴
        quantity: item.quantity,
        price: item.unit_price || item.products?.price || 0, // 실제 DB는 unit_price 사용
        totalPrice: item.total_price || (item.unit_price * item.quantity) || 0, // 실제 DB 스키마 반영
