...
export const createOrder = async (orderData, userProfile, depositName = null) => {
  try {
    ...
    let useKakaoAPI = false
    try {
      const { data: authUser, error: authError } = await supabase.auth.getUser()
      if (authError || !authUser.user) {
        console.log('Supabase Auth에 사용자가 없음 - 카카오 API 사용')
        useKakaoAPI = true
      }
    } catch (error) {
      console.log('Auth 확인 실패 - 카카오 API 사용')
      useKakaoAPI = true
    }

    // 카카오 사용자인 경우 또는 auth.users에 없는 경우 별도 API 사용
    if (useKakaoAPI || (user.provider && user.provider === 'kakao') || (user.email && user.email.includes('@temp.com')) || user.from_kakao) {
