...
export const getProducts = async (filters = {}) => {
  try {
    ...
    // ⚡ 단일 쿼리로 상품 + variant 데이터 동시 로드 (N+1 쿼리 해결)
    const { data, error } = await supabase
      ...
      // ⚠️ 개발 중: is_live, is_live_active 조건 제거 (모든 활성 상품 표시)
      // .eq('is_live', true)
    ...
    // ⚡ 클라이언트에서 variant 데이터 처리 (동기 처리, DB 쿼리 없음)
    const productsWithVariants = data.map(product => {
      try {
        // Variant 데이터 구조 정리 (getProductVariants와 동일한 로직)
        ...
        // product_variants 제거 (중복 데이터)
        const { product_variants, ...productWithoutRaw } = product
        ...
        console.error(`Variant 처리 실패 for product ${product.id}:`, error)
        return {
          ...product,
          options: [],
          variants: [],
