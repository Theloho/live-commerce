...
export const createOrder = async (orderData, userProfile, depositName = null) => {
  try {
    ...
    const itemData = {
      ...
      title: normalizedOrderData.title || '상품', // product_title → title (스키마 맞춤)
      quantity: normalizedOrderData.quantity,
      price: normalizedOrderData.price,
      total_price: normalizedOrderData.totalPrice || (normalizedOrderData.price * normalizedOrderData.quantity) // total → total_price (스키마 맞춤)
    }
  ...
  try {
    ...
    const ordersWithItems = finalData.map(order => ({
      ...
      items: order.order_items.map(item => ({
        ...
        title: item.title || item.product_title || item.products?.title || '상품', // title 우선, product_title은 fallback
        quantity: item.quantity,
        price: item.price || item.unit_price || item.products?.price || 0,
        totalPrice: item.total_price || item.total || (item.price * item.quantity) || 0, // total_price 우선, total은 fallback
