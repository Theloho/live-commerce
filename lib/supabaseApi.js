  ...
  try {
    ...
    const ordersWithItems = finalData.map(order => ({
      ...
      items: order.order_items.map(item => ({
        ...
        title: item.title || item.products?.title || '상품', // 신규 title 컬럼 우선, 없으면 products 테이블에서 가져옴
        quantity: item.quantity,
        price: item.price || item.unit_price || item.products?.price || 0, // 신규 price 컬럼 우선, 없으면 기존 unit_price 사용
        totalPrice: item.total || item.total_price || ((item.price || item.unit_price) * item.quantity) || 0, // 신규 total 컬럼 우선
    ...
    const ordersWithItems = await Promise.all(data.map(async order => {
      ...
      return {
        ...
        items: order.order_items.map(item => ({
          ...
          quantity: item.quantity,
          title: item.title || item.products?.title || '상품', // 신규 title 컬럼 우선
          price: item.price || item.unit_price || item.products?.price || 0, // 신규 price 컬럼 우선
          totalPrice: item.total || item.total_price || ((item.price || item.unit_price || 0) * item.quantity) || 0, // 신규 total 컬럼 우선
          variant: item.variant_title || null,
          sku: item.sku || null,
          selectedOptions: item.selected_options || {}
        })),
      ...
      if (order.payment_group_id && !processedGroupIds.has(order.payment_group_id)) {
        ...
        if (groupOrders.length > 1) {
          ...
          const groupOrder = {
            ...
            payment: (() => {
              // 상품 금액만 합산 - 새로운 스키마 필드 우선 사용
              const itemsTotal = groupOrders.reduce((sum, o) => {
                return sum + (o.items || []).reduce((itemSum, item) => {
                  return itemSum + (item.total || item.totalPrice || ((item.price || item.unit_price || 0) * item.quantity))
                }, 0)
              }, 0)
              // 배송비는 한 번만 추가
              const shippingFee = 4000
            ...
            order_payments: [(() => {
              const itemsTotal = groupOrders.reduce((sum, o) => {
                return sum + (o.items || []).reduce((itemSum, item) => {
                  // 새로운 스키마 필드를 우선 사용
                  const itemTotal = item.total || item.totalPrice || ((item.price || item.unit_price || 0) * item.quantity)
                  console.log('🔧 개별 아이템 계산:', {
                    title: item.title,
                    total: item.total,
              ...
              const shippingFee = 4000
              console.log('🔧 그룹 주문 payment.amount 계산 디버깅:', {
                itemsTotal,
                shippingFee,
                finalAmount: itemsTotal + shippingFee,
                groupOrders_length: groupOrders.length,
                groupOrders_items: groupOrders.map(o => ({
                  order_id: o.id,
                  items_count: o.items?.length || 0,
                  items_total: (o.items || []).reduce((sum, item) => sum + (item.total || item.totalPrice || ((item.price || item.unit_price || 0) * item.quantity)), 0)
                }))
              })
              return {
                ...order.payment,
