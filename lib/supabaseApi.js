...
export const updateProductInventory = async (productId, quantityChange) => {
  try {
    ...
    const { data: product, error: selectError } = await supabase
      .from('products')
      .select('id') // inventory_quantity 컬럼이 없으므로 id만 선택
      .eq('id', productId)
      .single()

    if (selectError) throw selectError

    // const newQuantity = Math.max(0, product.inventory_quantity + quantityChange) // inventory_quantity 컬럼이 없음

    // 재고 업데이트
    const { data, error } = await supabase
      .from('products')
      .update({
        // inventory_quantity: newQuantity, // 스키마에 없는 컬럼
        updated_at: new Date().toISOString()
      })
      .eq('id', productId)
      .select()
...
export const updateOrderItemQuantity = async (orderItemId, newQuantity) => {
  try {
    ...
    const fetchResponse = await fetch(`${supabaseUrl}/rest/v1/order_items?product_id=eq.${orderItemId}&select=quantity,total_price,id`, {
      headers: {
        'apikey': supabaseKey,
        'Authorization': `Bearer ${supabaseKey}`,
        'Content-Type': 'application/json'
