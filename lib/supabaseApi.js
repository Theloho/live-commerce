...
export const checkOptionInventory = async (productId, selectedOptions) => {
  try {
    ...
    for (const option of productOptions) {
      ...
      const matchedValue = values.find(v => v.name === selectedValue)
      ...
      const inventory = matchedValue.inventory || 0
      minInventory = Math.min(minInventory, inventory)
    }

    return {
      available: minInventory > 0,
...
export const createOrderWithOptions = async (orderData, userProfile, depositName = null) => {
  try {
    ...
    const itemData = {
      ...
      options: normalizedOrderData.selectedOptions || {}, // ⭐ options 컬럼에 저장
    ...
    // 6. 재고 차감 (옵션별 RPC 함수 사용)
    if (normalizedOrderData.selectedOptions && Object.keys(normalizedOrderData.selectedOptions).length > 0) {
      logger.debug('🔧 옵션별 재고 차감 시작 (RPC)')

      // 각 옵션별로 RPC 함수 호출
      for (const [optionName, optionValue] of Object.entries(normalizedOrderData.selectedOptions)) {
        try {
          await updateOptionInventoryRPC(
            normalizedOrderData.id,
            optionName,
            optionValue,
            -normalizedOrderData.quantity
          )
          logger.info(`✅ 옵션 재고 차감 성공: ${optionName} = ${optionValue}`)
        } catch (inventoryError) {
          console.error(`❌ 재고 차감 실패: ${optionName} = ${optionValue}`, inventoryError)
          // 재고 차감 실패 시 주문 롤백
      ...
      logger.info('✅ 모든 옵션 재고 차감 완료')
    } else {
      // 옵션이 없는 상품은 기존 방식으로 재고 차감
