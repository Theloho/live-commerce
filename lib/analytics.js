 ...
 */
export const trackPurchase = (order, user = null) => {
  if (!isGALoaded()) return
  ...
  window.gtag('event', 'purchase', {
    // 기본 거래 정보
    transaction_id: order.customer_order_number || order.id,
    value: order.total_amount || order.payment?.amount || 0,
    currency: 'KRW',
    shipping: order.shipping_fee || 0,

    // 사용자 정보 (Custom Parameters)
    user_id: user?.id || order.user_id,
    customer_name: order.shipping_name || user?.name || user?.nickname || '게스트',
