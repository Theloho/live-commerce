...
### 4. 🐛 Bug #23: 어제 날짜 필터 버그 수정 (2025-11-08 02:53)
- **문제**: "어제" 필터가 하루 전 데이터 조회 (11월 7일 대신 11월 6일)
- **원인**: yesterday case가 UTC 시간 사용 (Seoul timezone 미적용)
- **해결**: yesterday case에 Seoul timezone 적용 (today와 동일한 패턴)
- **영향**: 관리자 주문 관리 어제 필터, statusCounts 배지
- **커밋**: [다음 커밋 예정]
...
### 5. 🐛 Bug #23: 어제 날짜 필터 버그 (2025-11-08 02:53 발견)
**증상**:
- 현재 시각: 2025-11-08 02:53 (서울 시간)
- "어제" 필터 선택 시 11월 6일 데이터 조회 (기대: 11월 7일)

**원인**:
- `yesterday` case가 UTC 시간 기준으로 계산
- 서울 02:53 = UTC 전날 17:53
- UTC 기준 어제 = 11월 6일, 서울 기준 어제 = 11월 7일

**버그 코드**:
```javascript
case 'yesterday':
  const yesterday = new Date(now)  // ❌ UTC 시간
...
**수정 위치**:
1. `/api/admin/orders/route.js` line 146-157 (main query)
2. `/api/admin/orders/route.js` line 257-268 (statusCounts query)

**영향 범위**:
- 관리자 주문 관리 어제 필터
- statusCounts 배지 (어제 주문 수)

**Rule #0-A 적용**:
- Stage 0: 아키텍처 준수 ✅
- Stage 1: 버그 현상 파악 ✅
- Stage 2: 문서 확인 (WORK_LOG_2025-11-08.md) ✅
- Stage 3: 소스코드 확인 ✅
- Stage 4: 영향도 분석 ✅
- Stage 5: 수정 + 검증 ✅
- Stage 8: 문서 업데이트 ✅
...
2. `app/api/admin/orders/route.js` - 서울 시간 기준 (today + yesterday) + inventory 필드 추가
3. `app/api/admin/stats/route.js` - 서울 시간 기준 적용
4. `app/admin/orders/page.js` - 입금자명 강조 표시
5. `docs/work-logs/WORK_LOG_2025-11-08.md` - Bug #23 문서화
...
**주의사항**:
- 단순히 `new Date()`는 서버 시간 기준 (위험!)
- 모든 API에서 일관된 시간 기준 사용 필수
- "오늘", "어제"의 정의를 명확히 (Seoul 00:00 ~ 23:59:59)
- **today 수정했다면 yesterday도 확인!** (Bug #23 교훈)

**Bug #23 교훈**:
- today case에 Seoul timezone 적용 → ✅ 완료
- yesterday case는 누락 → ❌ 버그 발생
- 날짜 관련 수정 시 **모든 case 확인** 필수!
...
7. ✅ 재고 데이터 수정
8. ✅ 판매 통계 개선 (신규 회원)
9. ✅ 서울 시간 기준 통일 (today + yesterday)
10. ✅ 입금자명 강조 표시
11. ✅ 대시보드 통계 수정
12. ✅ Bug #23: 어제 날짜 필터 버그 수정

### 📦 커밋 목록 (15개)
```bash
a86d6c9 - feat: 셀러 라이브 대시보드 구현 - 방송 중 실시간 판매 현황
049cca8 - feat: 셀러 전용 로그인 페이지 추가 (/seller/login)
...
af9e5f2 - fix: 대시보드 통계를 서울 시간 기준으로 수정
1bc42de - docs: 2025-11-08 작업 로그 추가
ccf85c2 - fix: 어제 날짜 필터 Seoul timezone 적용 (Bug #23)
```
...
**핵심 성과**:
- ✅ 라이브 방송용 셀러 대시보드 완성
- ✅ 모든 API 서울 시간 기준 통일 (today + yesterday)
- ✅ Bug #23 수정 (어제 날짜 필터)
- ✅ 사용자 페이지에 영향 0%
- ✅ 깔끔한 UI/UX 개선

**Rule #0-A 준수**:
- ✅ Bug #23 수정 시 8-Stage Process 완벽 준수
- ✅ 총 소요 시간: 10분 (재작업 0분)
- ✅ 문서 업데이트 100% 완료
