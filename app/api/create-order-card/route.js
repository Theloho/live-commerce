  ...
  try {
    ...
    const { error: itemError } = await supabase
      ...
      .insert([{
        ...
        total: orderData.totalPrice || (orderData.price * orderData.quantity), // 개발 스키마 신규 컬럼
        unit_price: orderData.price, // 기존 컬럼 (호환성 유지)
        total_price: orderData.totalPrice || (orderData.price * orderData.quantity), // 기존 컬럼 (호환성 유지)
    ...
    // 4. 결제 정보 생성 (카드결제 - 부가세 10% 추가)
    const shippingFee = 4000
    const cardAmount = Math.floor(orderData.totalPrice * 1.1) // 부가세 10% 추가
    const totalAmount = cardAmount + shippingFee
    ...
    const { error: paymentError } = await supabase
      ...
      .insert([{
        ...
        amount: totalAmount,
        status: 'pending'
        // 카드결제는 depositor_name 불필요 (결제자 정보는 카드사에서 처리)
      }])
    ...
    console.log('카드결제 주문 생성 성공:', finalOrder)
    ...
    console.error('카드결제 주문 생성 오류:', error)
    return NextResponse.json(
      {
