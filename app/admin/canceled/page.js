...
export default function AdminCanceledPage() {
  ...
  const loadOrders = async () => {
    try {
      ...
      // ⚡ Service Role API 호출 (날짜 필터 + 취소된 주문만)
      let url = `/api/admin/orders?adminEmail=${encodeURIComponent(adminUser.email)}&status=cancelled`

      if (isOrderNumber || isUUID) {
        // 🎯 DB 직접 검색 (주문번호/UUID)
        url += `&search=${encodeURIComponent(searchTerm)}`
        console.log('🎯 [취소내역] DB 직접 검색:', searchTerm)
      } else {
      ...
      console.log('✅ API에서 가져온 취소된 주문:', allOrders.length, '개')
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">❌ 취소 내역</h1>
          <p className="text-sm text-gray-600 mt-1">
            총 {filteredOrders.length}건의 취소된 주문
            {dateRange !== 'all' && (
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        ...
        {filteredOrders.length === 0 && !loading && (
          <div className="text-center py-12">
            <p className="text-gray-500">조건에 맞는 취소된 주문이 없습니다.</p>
          </div>
        )}
      </div>
